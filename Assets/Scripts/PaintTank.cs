...
public class PaintTank : MonoBehaviour
{
    ...
    [SerializeField]
    float _maxLinearVelocity = 1f;

    [SerializeField]
    float _rotateSpeed = 1f;

    [SerializeField]
    float _maxAngularVelocity = 1f;

    [SerializeField]
    float _turretRotateSpeed = 30f;

    [SerializeField]
    Color _initializePaintColor = Color.red;

    [SerializeField]
    float _bulletSpeed = 10f;

    [SerializeField]
    float _paintMinSize = 1.0f;

    [SerializeField]
    float _paintMaxSize = 1.5f;
    ...
    public void Initialize()
    {
        ...
        {
            _bulletMaterial = new Material(_bulletPrefab.GetMaterial())
            {
                color = _initializePaintColor
            };
        }
    }
    ...
    void UpdateManually(float dt)
    {
        ...
        _turretAngle = Mathf.Clamp(
            _turretAngle + _turretRotateDirection * _turretRotateSpeed * dt,
            0,
            70f
        );
        _turret.localRotation =
            _initialTurretRotation * Quaternion.AngleAxis(_turretAngle, Vector3.right);
